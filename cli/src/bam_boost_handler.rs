...
/// An unsigned claim transaction built for a single epoch, plus the metadata
/// an external signer / automation needs to route and sign it.
struct UnsignedTx {
    epoch: u64,
    claimant: Pubkey,
    amount_lamports: u64,
...
impl UnsignedTx {
    fn to_json(&self) -> Value {
        json!({
            "epoch": self.epoch,
            "claimant": self.claimant.to_string(),
            "amount_lamports": self.amount_lamports,
            "recent_blockhash": self.recent_blockhash,
            "uses_nonce": self.uses_nonce,
            "assert_deploy_slot": self.assert_deploy_slot,
...
/// The raw wire encodings of an unsigned transaction, as returned by the
/// transaction-building path.
struct BuiltUnsignedTx {
    recent_blockhash: String,
    base58: String,
    base64: String,
}
...
impl BamBoostCliHandler {
    ...
    /// of `{epoch, claimant, amount, ..., unsigned_tx_base58/base64}` objects
    ...
    async fn claim(&self, cluster: &str, epoch: u64) -> anyhow::Result<()> {
        ...
        match self.check_eligibility(cluster, epoch).await? {
            Eligibility::Eligible(ec) => {
                let unsigned =
                    self.build_claim(epoch, &ec, assert_slot, self.nonces.first().copied())?;
    ...
    async fn claim_all(&self, cluster: &str, first_epoch: Option<u64>) -> anyhow::Result<()> {
        ...
        for (i, (epoch, ec)) in eligible.iter().enumerate() {
            ...
            match self.build_claim(*epoch, ec, assert_slot, nonce) {
                Ok(unsigned) => {
                    counters.claimed += 1;
        ...
        // Emit in ascending epoch order, in one batch (a single JSON array in
        // json mode) so automation gets one document.
        unsigned_txs.sort_by_key(|t| t.epoch);
    ...
    fn build_claim(
        ...
        // In unsigned/print mode, carry the built transaction + metadata up so
        // the caller can emit it (base58 lines or a JSON manifest).
        let unsigned = built.map(|b| UnsignedTx {
            epoch,
            claimant: ec.claimant,
            amount_lamports: ec.amount,
            recent_blockhash: b.recent_blockhash,
            uses_nonce: nonce.is_some(),
            assert_deploy_slot: assert_slot,
            base58: b.base58,
            base64: b.base64,
        });
    ...
    ///   - Returns its base58 + base64 wire encodings and blockhash via
    ///     [`BuiltUnsignedTx`] (the caller decides how to emit it)
    ///
    ...
    fn process_transaction(
    ...
    ) -> anyhow::Result<Option<BuiltUnsignedTx>> {
        let rpc_client = self.get_rpc_client();
        ...
        if self.should_print_tx() {
            ...
            return Ok(Some(BuiltUnsignedTx {
...
#[cfg(test)]
mod tests {
    use super::{base64_encode, parse_pubkey_list, resolve_nonce_specs};
    use solana_pubkey::Pubkey;
